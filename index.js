...
function addGamesToPage(games) {
  ...
  games.forEach((game) => {
    ...
    element.innerHTML = `
    <img src=${game.img} height="150"></img>
    <p>${game.name}</p>
...
let displayUnfundedGames = 
    `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames}. Currently, ${unfundedGames.length==1 ? "1 game" : unfundedGames.length + " games"}
    We need your help to fund these amazing games!`
// create a new DOM element containing the template string and append it to the description container
let element = document.createElement("p")
element.innerHTML = displayUnfundedGames;
 ...
 * 
 * 
 * Bonus: Add a search bar that filters based on a specific game
 * 
 * Skills used: filter(), eventListener
 */
...
function performSearch() {
    ...
    const similarGames = GAMES_JSON.filter((game) => {
        return game.name.toLowerCase().includes(searchText);
    });
    deleteChildElements(gamesContainer);
    addGamesToPage(similarGames);
}
